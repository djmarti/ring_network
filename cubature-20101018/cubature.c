...
static double errMax(unsigned func_dim, const esterr *ee)
{
     double errmax = 0;
     unsigned k;
     for (k = 0; k < func_dim; ++k)
...
typedef struct {
     hypercube h;
     unsigned splitDim;
     unsigned func_dim; /* dimensionality of vector integrand */
     esterr *ee; /* array of length func_dim */
     double errmax; /* max ee[k].err */
} region;

static region make_region(const hypercube *h, unsigned func_dim)
{
     region R;
     R.h = make_hypercube(h->dim, h->data, h->data + h->dim);
     R.splitDim = 0;
     R.func_dim = func_dim;
     R.ee = R.h.data ? (esterr *) malloc(sizeof(esterr) * func_dim) : NULL;
     return R;
}
...
static int cut_region(region *R, region *R2)
{
     ...
     *R2 = *R;
     ...
     R2->ee = (esterr *) malloc(sizeof(esterr) * R2->func_dim);
     return R2->ee == NULL;
}
...
typedef int (*evalError_func)(struct rule_s *r,
			      unsigned func_dim, integrand_v f, void *fdata,
			      unsigned nR, region *R);
typedef void (*destroy_func)(struct rule_s *r);


typedef struct rule_s {
     unsigned dim, func_dim;         /* the dimensionality & number of functions */
     ...
     double *vals; /* num_regions * num_points * func_dim */
...
static int alloc_rule_pts(rule *r, unsigned num_regions)
{
     if (num_regions > r->num_regions) {
	  ...
	  r->pts = (double *) malloc(sizeof(double) * 
				     (num_regions
				      * r->num_points * (r->dim + r->func_dim)));
	  if (r->func_dim + r->dim > 0 && !r->pts) return FAILURE;
...
static rule *make_rule(size_t sz, /* >= sizeof(rule) */
		       unsigned dim, unsigned func_dim, unsigned num_points,
		       evalError_func evalError, destroy_func destroy)
{
     ...
     r->pts = r->vals = NULL;
     r->num_regions = 0;
     r->dim = dim; r->func_dim = func_dim; r->num_points = num_points;
     r->evalError = evalError;
     r->destroy = destroy;
     return r;
}

/* note: all regions must have same func_dim */
static int eval_regions(unsigned nR, region *R, 
			integrand_v f, void *fdata, rule *r)
{
     unsigned iR;
     if (nR == 0) return SUCCESS; /* nothing to evaluate */
     if (r->evalError(r, R->func_dim, f, fdata, nR, R)) return FAILURE;
     for (iR = 0; iR < nR; ++iR)
	  R[iR].errmax = errMax(R->func_dim, R[iR].ee);
     return SUCCESS;
}
...
static int rule75genzmalik_evalError(rule *r_, unsigned func_dim, integrand_v f, void *fdata, unsigned nR, region *R)
{
     ...
     f(dim, npts, pts, fdata, func_dim, vals);
...
static rule *make_rule75genzmalik(unsigned dim, unsigned func_dim)
{
     rule75genzmalik *r;
				 ...
				 unsigned nR, region *R)
{
	...
	L. W. Fullerton, Bell Labs, Nov. 1981. */
     ...
     f(1, npts, pts, fdata, func_dim, vals);
...
static rule *make_rule15gauss(unsigned dim, unsigned func_dim)
{
     if (dim != 1) return NULL; /* this rule is only for 1d integrals */

     return make_rule(sizeof(rule), dim, func_dim, 15,
...
typedef struct {
     ...
     heap_item *items;
     unsigned func_dim;
     esterr *ee; /* array of length func_dim of the total integrand & error */
} heap;
...
static heap heap_alloc(unsigned nalloc, unsigned func_dim)
{
     heap h;
     unsigned i;
     h.n = 0;
     ...
     h.items = 0;
     h.func_dim = func_dim;
     h.ee = (esterr *) malloc(sizeof(esterr) * func_dim);
     if (h.ee) {
	  for (i = 0; i < func_dim; ++i) h.ee[i].val = h.ee[i].err = 0;
...
{
     int insert;
     unsigned i, func_dim = h->func_dim;
...
static heap_item heap_pop(heap *h)
{
     ...
     {
	  unsigned j, func_dim = h->func_dim;
	  for (j = 0; j < func_dim; ++j) {
...
static int ruleadapt_integrate(rule *r, unsigned func_dim, integrand_v f, void *fdata, const hypercube *h, unsigned maxEval, double reqAbsError, double reqRelError, double *val, double *err, int parallel)
{
     unsigned numEval = 0;
     heap regions;
     unsigned i, j;
     ...
     regions = heap_alloc(1, func_dim);
     if (!regions.ee || !regions.items) goto bad;

     ee = (esterr *) malloc(sizeof(esterr) * func_dim);
     if (!ee) goto bad;
     
     nR_alloc = 2;
     R = (region *) malloc(sizeof(region) * nR_alloc);
     if (!R) goto bad;
     R[0] = make_region(h, func_dim);
     if (!R[0].ee
     ...
     while (numEval < maxEval || !maxEval) {
	  for (j = 0; j < func_dim && (regions.ee[j].err <= reqAbsError
	  ...
	  if (parallel) { /* maximize potential parallelism */
		  ...
		  N, and it is also much simpler.] */
	       unsigned nR = 0;
	       for (j = 0; j < func_dim; ++j) ee[j] = regions.ee[j];
	       do {
		    if (nR + 2 > nR_alloc) {
			 nR_alloc = (nR + 2) * 2;
		    ...
		    for (j = 0; j < func_dim; ++j) ee[j].err -= R[nR].ee[j].err;
		    ...
		    if (j == func_dim) break; /* other regions have small errs */
     ...
     for (j = 0; j < func_dim; ++j) val[j] = err[j] = 0;  
...
static int integrate(unsigned func_dim, integrand_v f, void *fdata, 
		     ...
		     double *val, double *err, int parallel)
{
     ...
     if (func_dim == 0) /* nothing to do */ return SUCCESS;
     if (dim == 0) { /* trivial integration */
	  f(0, 1, xmin, fdata, func_dim, val);
	  for (i = 0; i < func_dim; ++i) err[i] = 0;
	  return SUCCESS;
     }
     r = dim == 1 ? make_rule15gauss(dim, func_dim)
 	          : make_rule75genzmalik(dim, func_dim);
     if (!r) { 
	  for (i = 0; i < func_dim; ++i) {
	       val[i] = 0;
	       err[i] = HUGE_VAL; 
     ...
     status = !h.data ? FAILURE
	  : ruleadapt_integrate(r, func_dim, f, fdata, &h,
...
int adapt_integrate_v(unsigned func_dim, integrand_v f, void *fdata, 
		      unsigned dim, const double *xmin, const double *xmax, 
		     unsigned maxEval, double reqAbsError, double reqRelError, 
		      double *val, double *err)
{
     return integrate(func_dim, f, fdata, dim, xmin, xmax, 
	       ...
	       unsigned func_dim, double *fval)
{
     ...
     /* printf("npt = %u\n", npt); */
     for (i = 0; i < npt; ++i) {
	  d->f(ndim, x + i*ndim, d->fdata, func_dim, fval1);
	  for (k = 0; k < func_dim; ++k) fval[k*npt + i] = fval1[k];
     }
}

int adapt_integrate(unsigned func_dim, integrand f, void *fdata, 
		    unsigned dim, const double *xmin, const double *xmax, 
		    unsigned maxEval, double reqAbsError, double reqRelError, 
		    double *val, double *err)
{
     ...
     if (func_dim == 0) return SUCCESS; /* nothing to do */     
     
     d.f = f; d.fdata = fdata;
     d.fval1 = (double *) malloc(sizeof(double) * func_dim);
     if (!d.fval1) {
	  unsigned i;
	  for (i = 0; i < func_dim; ++i) {
	       val[i] = 0;
     ...
     ret = integrate(func_dim, fv, &d, dim, xmin, xmax, 
...
int main(int argc, char **argv)
{
     ...
     /* parse: e.g. "x/y/z" is treated as func_dim = 3, which_integrand={x,y,z} */
     if (argc <= 3) {
	  integrand_fdim = 1;
